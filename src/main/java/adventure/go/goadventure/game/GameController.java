...
public class GameController {
    ...
    public Map<String, Object> getGameStepAndChoices(@RequestHeader("Authorization") String token, @PathVariable Integer id) {
        ...
        Optional<Session> latestSessionOpt = sessionRepository.findLatestByGameId(id, userId);
        if (latestSessionOpt.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No active session found for the game.");
        }

        Session latestSession = latestSessionOpt.get();
        ...
        // Fetch choices for the current step
        String sql = "SELECT c.id_choice, c.text AS choice_text, c.id_next_step " +
                "FROM public.\"Choice\" c " +
                "JOIN public.\"Step_Choices\" sc ON c.id_choice = sc.id_choice " +
                "WHERE sc.id_step = :id_step";
        MapSqlParameterSource params = new MapSqlParameterSource().addValue("id_step", currentStepId);

        List<Map<String, Object>> choices = jdbcTemplate.query(sql, params, (rs, rowNum) -> {
        ...
        stepData.put("text", step.getText());
        stepData.put("longitude", step.getLongitude());
        stepData.put("latitude", step.getLatitude());
        stepData.put("id_step", step.getId_step());
        stepData.put("choices", choices);

        response.put("step", stepData);
